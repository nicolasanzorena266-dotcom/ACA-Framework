...
def _state_summary(state: CognitiveState | None) -> Dict[str, Any]:
    ...
    return {
        ...
        "mission_decision": _mission_decision_summary(state),
        "fact_keys": sorted(state.facts.keys()),
    }


def _mission_decision_summary(state: CognitiveState) -> Dict[str, Any]:
    """Expose the MissionManager transition decision for this turn
    (ACA-305B/ACA-305D): proposals considered, the winning proposal (if
    any), acceptance/rejection with reason, mission_before/mission_after,
    and any topic_stack effect -- so every mission-authority decision is
    inspectable the same way output/verbalization decisions already are.
    """
    decision = state.facts.get("mission_transition_decision")
